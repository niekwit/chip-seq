...
sampleSheet <- sampleSheet %>%
  ...
  mutate(
    Condition = paste0(csv$genotype, "_", csv$factor, "_", csv$treatment)
  ) %>% #simplifies analysis
...
for (condition in conditions) {
  ...
  for (i in 1:contrasts) {
    ...
    if (sum(dbs$Fold > 0) > 1 | sum(dbs$Fold < 0) > 1) {
      ...
      print(paste0(
        "Exporting differential binding sites to a bed file for ",
        contrast[2],
        " vs ",
        contrast[3]
      ))
      ...
      pathways_all <- enrichPathway(df$ENTREZID, organism = organism)

      tryCatch(
        {
          file <- file.path(
            dir.out,
            paste0("dotplot_all_peaks-", condition, ".pdf")
          )
          pdf(file)

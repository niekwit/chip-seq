...
for condition in conditions:
    ...
    with open(log, "a") as log:
        ...
        if k > len(beds):
            k = len(beds)
            log.write(
                f"Warning: k is larger than the number of bed files for {condition}. Setting k to {k}.\n"
            )
        ...
        bed_out_intermediate = [
            x for x in out_bed_files_intermediate if condition in x
        ][0]
        consensus_peaks.to_csv(
            bed_out_intermediate, sep="\t", header=False, index=False
        )
